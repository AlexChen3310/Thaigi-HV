...
extension String {
    ...
    var boolValue: Bool {
        ...
        return !falseValues .contains(self.lowercased());
    }
    var isNotEmpty:Bool {
    ...
    func getSize(font: UIFont, height: CGFloat) -> CGSize{
        let rect = NSString(string: self).boundingRect(
            ...
            context: nil)
        return CGSize(width: rect.size.width, height: height)
    }
    func canFindCharacter(_ character: String) -> Bool{
        
        if let _ = self.range(of: character){
            return true
        }else{
            return false
        }
    ...
    func filterDateString() -> String {
        ...
        if (self.range(of: "/") != nil) {
            let scoreArray = self.split(separator: "/")
            result = String(scoreArray.last! + " / " + scoreArray.first!)
        }
    ...
    var digitsAfterDot: Int{
        ...
        guard let _ = splitString.last else {
            return 0
        }
        return splitString.last?.count ?? 0
    }
    ...
    var localizable: String {
        return localized(withKey: self, withLocalizationFileNmae: "zh-Hans")
    }
    ...
    func localized(withKey key: String, withLocalizationFileNmae localizationFileNmae: String) -> String {
        // 取得 Bundle 下的的多國語系檔
        guard let path = Bundle.main.path(forResource: localizationFileNmae, ofType: "lproj") else{
            return key
        }
    ...
    func aesEncrypt(key: String, iv: String) -> String? {
        ...
        do {
            ...
            // 用AES的方式將Data加密
            let aecEnc: AES = try AES(key: "IF8stOGviqR76oUH".bytes, blockMode: CBC(iv: iv.bytes))
    ...
    func aesDecrypt(key: String, iv: String) -> String? {
        ...
        do {
            ...
            // 用AES方式將Data解密
            let aesDec: AES = try AES(key: "IF8stOGviqR76oUH".bytes, blockMode: CBC(iv: iv.bytes))
...
public class GBig {
    ...
    public var big5: String {
        return GBig.traditionalize(key)
    }
    public var gb: String {
        return GBig.simplify(key)
    }
}
